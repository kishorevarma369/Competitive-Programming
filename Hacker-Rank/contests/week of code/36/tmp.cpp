...
void rook_check(vector < vector<char> > &board,int i,int j)
{
  for(int k=j+1;k<8;k++)
  {
    if(valueat(board,i,k)!='#')
    {
      if(valueat(board,i,k)=='k') cou+=2;
      break;
    }
  }
  for(int k=j-1;k>=0;k--)
  ...
  for(int k=i-1;k>=0;k--)
  {
    if(valueat(board,k,j)!='#')
    {
      if(valueat(board,k,j)=='k') cou+=2;
      break;
    }
  }
  for(int k=i+1;k<8;k++)
...
void bishop_check(vector < vector<char> > &board,int i,int j)
{
  ...
  for(;k<8&&l<8;k++,l++)
  {
    if(valueat(board,k,l)!='#')
    {
      if(valueat(board,k,l)=='k') cou+=2;
      break;
    }
  }
  k=i+1;
...
int waysToGiveACheck(vector < vector<char> >& board)
{
  ...
  for(;j<8;j++)
  {
    if(board[i][j]=='P')
    {
      ...
      if(board[k][j]=='#')
      {
        night_check(board,k,j);
        bishop_check(board,k,j);
        rook_check(board,k,j);
      }
      board[i][j]='P';
    }
  }

...
void format_string()
{
    ...
    else if(len-to_remove==1) tans=0;
    // {
    //     if(len>1)
    //     {
    //         while(a.back()<b.back()){
    //             if(a[len-1]==a[len-2]) ans=add(ans,a.back()-'0');
    //             a.back()++;
    //         }
    //         if(a[len-1]==a[len-2]) ans=add(ans,a.back()-'0');
    //     }
    ...
    i=to_remove;
    while(i>=to_remove&&a[i]<b[i])
    {
        if(len-i-2>=0) ans=add(ans,add(multiply(multiply((a[i]-'0'),ten_power[len-i-2]),ten_power[len-i-2]),loss[len-i-1]));
        if(i>0&&a[i-1]==a[i])
        {
            ans=add(ans,multiply(multiply((1),ten_power[len-i-1]),tans));
            // ans=add(ans,add(multiply(multiply((a[i]-'0'),ten_power[len-i-2]),ten_power[len-i-2]),loss[len-i-1]));
        }
        tans=add(tans,ten_power[len-1-i]);
        a[i]++;
    }
    for(;i>0;i--)
    {
        if(a[i]==a[i-1]){
            // ans=add(ans,multiply(multiply(tans,ten_power[len-1-k]),(a[k]-'0')));
            ans=add(ans,multiply(multiply((a[i]-'0'),ten_power[len-i-1]),tans));
        }
    }
    ll tlen=len-1;
    i=to_remove+1;
